...
interface WorkflowStep {
  ...
  name: string;
  description?: string;
  type: 'inspection' | 'photo' | 'signature' | 'form';
  order: number;
  required: boolean;
  config?: {
    fields?: Array<{
...
export default function WorkflowExecutionPanel({ 
  ...
  const { data: existingExecution } = useQuery({
    queryKey: [`/api/field-engineering/tasks/${taskId}/executions`],
    enabled: !!taskId && !isHydrated,
  });
  ...
  const createExecutionMutation = useMutation({
    mutationFn: async () => {
      return await apiRequest(`/api/field-engineering/tasks/${taskId}/executions`, {
        method: 'POST',
        body: {
  ...
  const updateExecutionMutation = useMutation({
    mutationFn: async ({ status, data, currentStepId }: { status: string; data: any; currentStepId?: string | null }) => {
      return await apiRequest(`/api/field-engineering/task-executions/${executionId}`, {
        method: 'PUT',
        body: {
          status,
          stepsCompleted: Object.values(data).filter((s: any) => s.completed).length,
    ...
    onSuccess: (data, variables) => {
      if (variables.status === 'completed') {
        ...
        queryClient.invalidateQueries({ queryKey: [`/api/field-engineering/tasks/${taskId}`] });
        toast({
