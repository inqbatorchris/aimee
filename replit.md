...
-   **Manual Splice Documentation System**: Desktop interface for documenting fiber-to-fiber splice connections with visual click-to-connect UI. Supports splice tray management, cable selection from connected nodes, fiber number selection (1-12), and TIA-598-C color standard dropdowns. Splice connections stored in `fiber_connections` table with both legacy integer columns (`left_cable_id`, `right_cable_id`) and new varchar columns (`cable_a_id`, `cable_b_id`) for proper cable ID tracking. The varchar columns store string cable IDs (e.g., "CBL-001-002") enabling accurate utilization aggregation.
-   **AI-Powered Splice Documentation Workflow**: Comprehensive voice-to-data system for documenting fiber-to-fiber splice connections in the field. Field engineers photograph splice trays and record voice descriptions, which are automatically transcribed via OpenAI Whisper and parsed by GPT-4 to extract structured splice data (cable IDs, fiber numbers, buffer tube colors). The workflow enables desktop review and verification before committing connections to fiber network nodes. Work items attach directly to fiber nodes via `workflowMetadata.fiberNodeId` (not address records). Architecture: Voice memo upload → AI processing (async) → Human review in `fiber_splice_documentation` step → Workflow completion → Webhook callback to `/api/fiber-network/save-splice-connections` → Splice connections saved to `fiber_connections` table with `cable_a_id`/`cable_b_id` columns. Implemented with `audio_recordings` table, `audioProcessingService` for AI extraction, field-app upload endpoints, and WorkflowStep UI component with auto-loading of AI-extracted data.
...
-   **Fiber Status Visibility System**: Real-time fiber utilization tracking across the network. Aggregates data from THREE sources: `fiber_terminations` (customer connections with cable_id, fiber_number, is_live status), `fiber_connections` (network splices via cable_a_id/cable_b_id columns), and `work_items` (reserved fibers). Backend API endpoints: `/cables/:id/fiber-status` for per-cable details, `/nodes/:nodeId/fiber-status` for per-node summary (includes splice counts), and `/cables/utilization` batch endpoint for table view. UI displays status badges (AVAILABLE in green, USED in blue, LIVE in amber, RESERVED in gray) in splice tray dialogs and cable dropdowns. Node detail sheet includes "Fibers" tab with cable utilization progress bars, color-coded (green < 50%, amber 50-80%, red > 80%). Cables table shows utilization column with real-time progress bars and tooltip showing breakdown (used, spliced, live, available). TIA-598-C color standards fully integrated throughout.
