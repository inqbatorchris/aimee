...
-   **AI Agent Workflows (Automation)**: Autonomous AI-driven automation with manual, webhook, and scheduled triggers, and various integrations.
    ...
    -   **Splynx Query System**: Comprehensive integration enabling agent workflows to query Splynx entities (customers, leads, support_tickets, scheduling_tasks) with advanced filtering and dual operation modes. **Count Mode**: Returns numeric counts for KPI tracking and Key Result auto-updates. **List Mode**: Returns full normalized records with IDs for iteration and work item creation. Filter system supports 9 operator types (equals, not_equals, contains, greater_than, less_than, greater_than_or_equal, less_than_or_equal, is_null, not_null) with variable interpolation for dynamic queries using context variables ({objectiveId}, {today}, etc.). **Customer Labels Filtering**: Full support for 24 customer fields including `customer_labels` (array of label objects with id, label, color). Uses searchable dropdown populated with actual labels from all customers (fetched via SplynxLabelService with 15-minute in-memory cache). Filters customer labels using exact matching (case-insensitive) via client-side filtering (e.g., "Out of contract", "FOC services"), eliminating typos and ensuring all labels are discoverable. **Date Range Filtering**: Supports configurable date range filtering with selectable date field (date_add or last_update). UI dropdown appears when a date range is selected and properly clears when range is removed to prevent stale values in incremental runs. Note: Splynx API may not support all date fields for all entities (e.g., last_update may not be supported for customers entity). Entity-specific schemas provide field metadata (name, label, type, operators, options) via `/api/integrations/splynx/schema/:entity` endpoint. Backend `SplynxService.queryEntities()` translates UI filters to Splynx API format with intelligent date field mapping per entity type, and applies post-fetch filtering for fields that require client-side processing. Supports incremental fetching using `lastSuccessfulRunAt` for delta queries. Optional Key Result auto-update enables direct KPI updates with set_value or increment modes. UI component `SplynxQueryBuilder` provides schema-driven field selection with dropdown options for enum fields. Result storage in context variables enables multi-step workflows and record iteration patterns. Credentials secured with zero logging of sensitive data. **Label Caching**: `SplynxLabelService` fetches all customer labels (up to 50k customers) on first request, caches for 15 minutes per organization, and gracefully falls back to stale cache on errors. Example use case: Query active customers with "Out of contract" label, count them, update a Key Result automatically on schedule.
...
-   **TipTap**: Rich text editing.    -   **Workflow Iteration & Work Item Creation**: Agent workflows support for_each loops to iterate over Splynx query results and create platform work items or Splynx tasks for each record. **for_each Step**: Takes a source variable (array from previous step like Splynx query results) and executes nested child steps for each item with `{currentItem}` context variable providing access to record properties (e.g., `{currentItem.id}`, `{currentItem.name}`). **create_work_item Step**: Creates platform work items with template-driven configuration supporting title, description, status, due date (+N days or YYYY-MM-DD), and external reference (stored in workflowMetadata JSONB). Supports {{variable}} syntax for dynamic values. **create_splynx_task Integration Action**: Creates Splynx scheduling tasks with customer linking via POST /api/2.0/admin/scheduling/tasks. UI features project/task type selector dropdown populated from `/api/integrations/splynx/projects` endpoint (fetches real Splynx project data), customer ID variable input supporting {{currentItem.id}} within loops, and proper numeric projectId persistence. Child step configuration UI provides full inline editing of work item titles, descriptions, log messages, and other parameters within for_each loop bodies. Example workflow: Query Splynx customers with specific labels → for_each customer → create work item titled "Follow up with {currentItem.name}" with external reference "splynx_customer_{currentItem.id}" → create Splynx task linked to customer ID.
