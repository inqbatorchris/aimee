...
-   **AI Agent Workflows (Automation)**: Autonomous AI-driven automation with manual, webhook, and scheduled triggers, and various integrations.
    ...
    -   **Splynx Query System**: Comprehensive integration enabling agent workflows to query Splynx entities (customers, leads, support_tickets, scheduling_tasks) with advanced filtering and dual operation modes. **Count Mode**: Returns numeric counts for KPI tracking and Key Result auto-updates. **List Mode**: Returns full normalized records with IDs for iteration and work item creation. Filter system supports 9 operator types (equals, not_equals, contains, greater_than, less_than, greater_than_or_equal, less_than_or_equal, is_null, not_null) with variable interpolation for dynamic queries using context variables ({objectiveId}, {today}, etc.). Entity-specific schemas provide field metadata (name, label, type, operators, options) via `/api/integrations/splynx/schema/:entity` endpoint. Backend `SplynxService.queryEntities()` translates UI filters to Splynx API format with intelligent date field mapping per entity type. Supports incremental fetching using `lastSuccessfulRunAt` for delta queries. Optional Key Result auto-update enables direct KPI updates with set_value or increment modes. UI component `SplynxQueryBuilder` provides schema-driven field selection with dropdown options for enum fields. Result storage in context variables enables multi-step workflows and record iteration patterns. Credentials secured with zero logging of sensitive data. Example use case: Query active customers, count them, update a "Total Customers" Key Result automatically on schedule.
